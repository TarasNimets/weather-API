...
public final class NowWeather {
    ...
    public static final void setNowWeather(Current current, NowWeatherModel model) {
        try {
            ...
            if (humidity >= 71 && humidity < 86) builder.append("normal humidity (").append(humidity).append(" %), ");
            if (humidity >= 86 ) builder.append("high humidity (").append(humidity).append(" %), ");
            if (wind < 2.8) builder.append("calm (").append(wind).append(" m/s)");
            if (wind >= 2.8 && wind < 8.4) builder.append("quiet wind (").append(wind).append(" m/s)");
            if (wind >= 8.4 && wind < 16.8) builder.append("light breeze (").append(wind).append(" m/s)");
            if (wind >= 16.8 && wind < 28) builder.append("weak wind (").append(wind).append(" m/s)");
            if (wind >= 28 && wind < 44.8) builder.append("moderate wind (").append(wind).append(" m/s)");
            if (wind >= 44.8 && wind < 58.8) builder.append("fresh wind (").append(wind).append(" m/s)");
            if (wind >= 58.8 && wind < 75.6) builder.append("strong wind (").append(wind).append(" m/s)");
            if (wind >= 75.6) builder.append("storm wind (").append(wind).append(" m/s)");
